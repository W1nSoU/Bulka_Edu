...
def _admin_study_keyboard(is_observer: bool = False, is_territorial: bool = False) -> InlineKeyboardMarkup:
    row3 = []
    is_readonly = is_observer or is_territorial
    if not is_readonly:
...
async def _show_admin_photo_menu(
    ...
    is_deleted = getattr(callback, "_message_deleted", False)

    if has_photo and not is_deleted and photo_path.exists():
        try:
    ...
    # Якщо повідомлення було текстовим (без фото) або видаленим, або редагування не вдалося:
    if message and not is_deleted:
        try:
...
async def dev_main_study_handler(callback: CallbackQuery):
    ...
    await _show_admin_photo_menu(
        ...
        _admin_study_keyboard(is_observer=is_observer, is_territorial=is_territorial)
    )
...
async def _send_text_screen(callback: CallbackQuery, text: str, reply_markup=None):
    """
    Helper for text-only screens (lists, search prompt, shop/manager filter lists):
    - If current message has a photo or was marked as deleted, deletes it and sends clean text.
    - Otherwise edits text in place.
    """
    message = callback.message
    if not message:
        return None
    if getattr(message, "photo", None) or getattr(callback, "_message_deleted", False):
        try:
            await message.delete()
        except Exception:
            pass
        return await message.answer(text, reply_markup=reply_markup)
    return await _edit_or_answer(message, text, reply_markup=reply_markup)
...
async def _build_managers_team_view(is_admin: bool, is_territorial: bool, user_id: int, page: int = 0, bot: Optional[Bot] = None):
    ...
    for idx, hr in enumerate(paginated_hrs, start=start + 1):
        ...
        is_trainee = hr.get("process") == "Керівник Стажер"
        if is_trainee:
            u_entry = await get_user_details(hr["uid"])
            cur_day = u_entry.get("current_block", 1) if u_entry else 1
            from database.positions import get_days_count_for_role
            total_days_mgr = await get_days_count_for_role("Керівник")
            btn_text = f"👔 {name} | 🎓 Стажер ({cur_day}/{total_days_mgr}) | {abbr}"
        else:
...
async def _build_dev_team_view(bot: Optional[Bot] = None, is_observer: bool = False) -> tuple[str, InlineKeyboardMarkup]:
...
async def developer_dev_team_menu(callback: CallbackQuery, state: FSMContext):
    ...
    text, kb = await _build_dev_team_view(bot=callback.bot, is_observer=is_obs)
...
async def developer_admin_view(callback: CallbackQuery, state: FSMContext):
    ...
    is_obs = await is_observer_user(callback.from_user.id)
    buttons = []
    if admin_uid != MAIN_DEVELOPER_ID and not is_obs:
...
def _users_staff_keyboard(is_admin: bool, is_territorial: bool, is_observer: bool = False) -> InlineKeyboardMarkup:
    buttons = [
        ...
        [
            InlineKeyboardButton(text="🎓 Стажери без керівника", callback_data="dev_users_interns_no_mgr"),
            InlineKeyboardButton(text="👷 Працівники без керівника", callback_data="dev_users_workers_no_mgr"),
        ],
        [
...
async def developer_users_menu(callback: CallbackQuery):
    ...
    await _show_admin_photo_menu(
        ...
        "img/admin/admin_spus.jpg",
        "👥 <b>Користувачі</b>\n\nОберіть категорію:",
        await _get_users_menu_kb(callback.from_user.id),
    )
...
async def developer_users_staff_menu(callback: CallbackQuery):
    ...
    await _show_admin_photo_menu(
        ...
        await _get_users_staff_kb(callback.from_user.id),
    )
...
async def developer_users_filters_menu(callback: CallbackQuery):
    ...
    await _show_admin_photo_menu(
        ...
        await _get_users_filters_kb(callback.from_user.id),
    )
...
async def _developer_show_users_list(callback: CallbackQuery, users: list, title: str, mode: str, page: int = 0, show_day: bool = True):
    ...
    for idx, user in enumerate(paginated_users, start=start_offset + 1):
        ...
        if display_role in ["Адміністратор", "Dev"]:
            job_title = "Адміністратор"
        elif display_role == "Керівник":
            job_title = "Керівник"
        elif display_role == "Територіал":
            job_title = "Територіал"
        elif display_role == "Наглядач":
            job_title = "Наглядач"
        elif is_worker:
            job_title = user.get('role') or "Не вказано"
        else:
            job_title = user.get('role') or "Стажер"
    ...
    back_target = "dev_users_staff" if mode in ("interns", "workers", "all", "interns_no_mgr", "workers_no_mgr") else "dev_users_filters"
...
async def developer_list_interns_no_manager(callback: CallbackQuery, page: int = 0):
    has_access, is_admin, is_territorial = await _check_access(callback)
    if not has_access: return
    all_interns = await get_all_interns()
    if is_territorial:
        all_interns = await _filter_users_for_territorial(callback.from_user.id, all_interns)
    from database.managers import is_user_without_store_manager
    users = []
    for u in all_interns:
        if await is_user_without_store_manager(u):
            users.append(u)
    await _developer_show_users_list(callback, users, "🎓 <b>Стажери без керівника</b>", "interns_no_mgr", page, show_day=True)

async def developer_list_workers_no_manager(callback: CallbackQuery, page: int = 0):
    has_access, is_admin, is_territorial = await _check_access(callback)
    if not has_access: return
    all_workers = await get_all_workers()
    if is_territorial:
        all_workers = await _filter_users_for_territorial(callback.from_user.id, all_workers)
    from database.managers import is_user_without_store_manager
    users = []
    for u in all_workers:
        if await is_user_without_store_manager(u):
            users.append(u)
    await _developer_show_users_list(callback, users, "👷 <b>Працівники без керівника</b>", "workers_no_mgr", page, show_day=False)
...
def _decode_back_callback(ret_code: str) -> str:
    ...
    if ret_code.startswith("mgr_team_"):
        mgr_uid = ret_code.split("_")[2]
        return f"dev_mgr_view:{mgr_uid}"
    if ret_code == "mgr_team":
        return "dev_manage_managers"
    return ret_code
...
async def developer_user_card_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await _show_user_profile_card(callback, user_id, state=state, back_callback=back_cb, ret_code=ret_code)
    try:
        await callback.answer()
    except Exception:
        pass
...
async def developer_user_card_back_handler(callback: CallbackQuery, state: FSMContext):
    """
    Обробник повернення з картки користувача.
    Гарантовано та чітко видаляє фото картки працівника!
    """
    try:
    ...
    # Не мутуємо frozen Pydantic об'єкт — просто виставляємо прапор
    callback._message_deleted = True
    ...
    ret_code = parts[1] if len(parts) > 1 else "menu"

    if ret_code == "srch":
        # Після пошуку повертаємось до меню «Користувачі», а не перезапускаємо пошук
        await developer_users_menu(callback)
    elif ret_code == "staff":
        await developer_users_staff_menu(callback)
    elif ret_code == "filters":
        await developer_users_filters_menu(callback)
    elif ret_code == "menu":
        await developer_users_menu(callback)
    elif ret_code.startswith("sh_"):
        sh_parts = ret_code.split("_")
        c_idx = int(sh_parts[1])
        s_idx = int(sh_parts[2])
        callback.data = f"dev_u_shop:{c_idx}:{s_idx}"
        await developer_users_shop_selected(callback)
    elif ret_code.startswith("mg_"):
        mgr_uid = int(ret_code.split("_")[1])
        callback.data = f"dev_u_mgr:{mgr_uid}"
        await developer_users_manager_selected(callback)
    elif ret_code.startswith("ct_"):
        c_idx = int(ret_code.split("_")[1])
        city = AVAILABLE_CITIES[c_idx] if c_idx < len(AVAILABLE_CITIES) else ""
        from database.users import get_all_users
        users = await get_all_users()
        city_users = [u for u in users if (u.get("city") or "").strip().lower() == city.lower()]
        await _developer_show_users_list(callback, city_users, f"🏙️ <b>Стажери: {city}</b>", f"city:{city}", 0)
    else:
        await developer_users_menu(callback)
...
async def developer_users_pagination_handler(callback: CallbackQuery):
    ...
    elif mode == "interns_no_mgr":
        await developer_list_interns_no_manager(callback, page)
    elif mode == "workers_no_mgr":
        await developer_list_workers_no_manager(callback, page)
    else:
...
async def developer_process_user_search(message: Message, state: FSMContext):
    ...
    if len(users) == 1:
        ...
        await _show_user_profile_card(message, intern_id, state=state, back_callback="dev_users_search", ret_code="srch")
    else:
        ...
        for u in display_users:
            ...
            user_id = u['user_id']
            real_role = await get_display_role(user_id)
            # Якщо це стажер — показуємо посаду, інакше — роль
            role_label = u.get('role', 'Стажер') if real_role == "Стажер" else real_role
            btn_text = f"👤 {full_name} | {role_label}"
...
async def _build_manager_card(manager_uid: int, is_admin: bool, is_territorial: bool, is_observer: bool = False, bot: Optional[Bot] = None):
    ...
    trainee_info = ""
    if mgr.get("process") == "Керівник Стажер":
        u_entry = await get_user_details(manager_uid)
        cur_day = u_entry.get("current_block", 1) if u_entry else 1
        from database.positions import get_days_count_for_role
        total_days_mgr = await get_days_count_for_role("Керівник")
        trainee_info = f"\n\n🎓 <b>Навчання:</b> Керівник Стажер\n📅 <b>Прогрес:</b> День {cur_day} з {total_days_mgr}"
    ...
    buttons = []
    if mgr.get("process") == "Керівник Стажер" and mgr.get("status") != "fired" and not is_observer:
        buttons.append([InlineKeyboardButton(text="📅 Навчальні дні", callback_data=f"dev_days_manage:{manager_uid}:mgr_team")])
...
async def dev_mgr_change_shops_start(callback: CallbackQuery, state: FSMContext):
    """Початок зміни магазинів керівника з підтягуванням поточних магазинів."""
    has_access, is_admin, is_territorial = await _check_access(callback)
    if not has_access:
        return
    manager_uid = int(callback.data.split(":")[1])
    ...
    current_shops = mgr.get("shops") or []
    if isinstance(current_shops, str):
        try:
            current_shops = json.loads(current_shops)
        except Exception:
            current_shops = [current_shops]
    current_shops = list(current_shops)
    
    # Зберігаємо стан: початкові магазини та поточний вибір
    await state.update_data(
        edit_mgr_uid=manager_uid,
        manager_city=city,
        original_shops=list(current_shops),
        selected_shops=list(current_shops)
    )
    await state.set_state(DeveloperStates.waiting_edit_manager_shops)
    
    kb = await _build_edit_shops_keyboard(city, current_shops, manager_uid)
    await _edit_or_answer(
        callback.message,
        f"🏪 Зміна магазинів для керівника <b>{html.escape(mgr.get('full_name') or '')}</b> (місто {city}):\n\n"
        f"Оберіть магазини (до 5):\n"
        f"<i>💡 Працівники відкріплених магазинів будуть автоматично передані Територіалу після збереження.</i>",
        reply_markup=kb
    )
    await callback.answer()
...
async def dev_mgr_edit_shop_done(callback: CallbackQuery, state: FSMContext):
    """Завершення зміни магазинів керівника з розрахунком дельти та переведенням."""
    manager_uid = int(callback.data.split(":")[1])
    data = await state.get_data()
    city = data.get("manager_city") or "Хмельницький"
    original_shops = data.get("original_shops", [])
    selected_shops = data.get("selected_shops", [])
    
    # 1. Оновлюємо список магазинів керівника
    await update_manager_shops(manager_uid, city, selected_shops)
    
    # 2. Розраховуємо різницю
    ...
    # 3. Відкріплені магазини: працівників і стажерів автоматично передаємо до Територіала міста
    from database.managers import (
    ...
    if removed_shops:
        await reassign_removed_shops_users_to_territorial(city, removed_shops, manager_uid)
        
    # 4. Додані магазини: перевіряємо, чи є співробітники під Територіалом/Адміном
    interns_c, workers_c = await count_unassigned_or_territorial_users_in_shops(city, added_shops, exclude_manager_uid=manager_uid)
    total_new = interns_c + workers_c
    
    if total_new > 0:
        # Зберігаємо дані для переведення при підтвердженні
        await state.update_data(transfer_added_shops=added_shops, transfer_mgr_uid=manager_uid, transfer_city=city)
        shops_joined = ", ".join(selected_shops)
        text = (
            f"✅ Магазини оновлено: <b>{html.escape(shops_joined)}</b>.\n\n"
            f"👥 У доданих магазинах знайдено <b>{interns_c}</b> стажерів та <b>{workers_c}</b> працівників (закріплених за Територіалом).\n"
...
async def dev_mgr_transfer_shops_yes(callback: CallbackQuery, state: FSMContext):
    ...
    manager_uid = int(callback.data.split(":")[1])
    data = await state.get_data()
    added_shops = data.get("transfer_added_shops")
    city = data.get("transfer_city")
    
    if not added_shops or not city:
        mgr = await get_manager_by_uid(manager_uid)
        if mgr:
            city = mgr.get("city") or "Хмельницький"
            added_shops = mgr.get("shops") or []
            if isinstance(added_shops, str):
                try:
                    added_shops = json.loads(added_shops)
                except Exception:
                    added_shops = [added_shops]
                    
    from database.managers import transfer_shop_users_to_manager
    transferred = await transfer_shop_users_to_manager(city, added_shops, manager_uid)
    await state.clear()
    await callback.answer(f"✅ Переведено {transferred} співробітників під керівництво!", show_alert=True)
...
async def developer_ack_analytics_menu(callback: CallbackQuery):
    """Головне меню аналітики ознайомлення — одразу відкриває список дат оновлень матеріалів."""
    await developer_ack_events_list_view(callback, page=0)


async def developer_ack_events_list_view(callback: CallbackQuery, page: int = 0):
    """Список дат змін навчальних матеріалів з пагінацією."""
    if not await _ensure_developer(callback, allow_observer=True):
        return

    # Якщо викликано з callback_data виду dev_ack_events_list:{page}
    if callback.data and callback.data.startswith("dev_ack_events_list:"):
        try:
            page = int(callback.data.split(":")[1])
        except (IndexError, ValueError):
            page = 0

    from database.material_notifications import get_all_material_events, prune_old_material_events
    await prune_old_material_events(days=90)
    events = await get_all_material_events(days_limit=90)
    ...
    for ev in paginated_events:
        ...
        buttons.append([InlineKeyboardButton(
            text=btn_text,
            callback_data=f"dev_ack_ev:{ev['id']}:0"
        )])
    ...
    text = (
        "📋 <b>Аналітика ознайомлення з матеріалами</b>\n"
        "───────────────────\n"
        "Оберіть дату та подію зміни матеріалу для перегляду звіту:"
    )
    await _edit_or_answer(callback.message, text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    await callback.answer()
...
async def developer_ack_event_card_view(callback: CallbackQuery):
    """Картка події: загальний прогрес, інлайни магазинів, кнопки дій."""
    if not await _ensure_developer(callback, allow_observer=True):
        return

    # Format: dev_ack_ev:{event_id}:{page}
    parts = callback.data.split(":")
    ...
    desc = event.get('description', '') or 'не вказано'
    if len(desc) > 120:
        desc_preview = desc[:117] + "..."
    else:
        desc_preview = desc
    ...
    text = (
        ...
        f"📝 Зміна: <i>{desc_preview}</i>\n"
        f"🕒 Дата розсилки: <b>{dt_formatted}</b>\n\n"
        f"📊 <b>Загальний прогрес:</b>\n"
        f"• Всього в розсилці: <b>{overall_stats['total']}</b> осіб\n"
        f"• ✅ <b>Ознайомились:</b> {overall_stats['acknowledged_count']} ({overall_stats['ack_percent']}%)\n"
        f"  └ ⏳ з них із запізненням (>72 год): <b>{overall_stats['acknowledged_late_count']}</b>\n"
        f"• ❌ <b>Не ознайомились:</b> {overall_stats['unacknowledged_count']} ({overall_stats['unack_percent']}%)\n"
        f"───────────────────\n"
        f"Оберіть магазин для перегляду списку співробітників:"
    )

    buttons = [
        [InlineKeyboardButton(text="📝 Зміст змін", callback_data=f"dev_ack_changes:{event_id}:{page}")],
        [InlineKeyboardButton(text="📥 Вивантажити результати (XLSX)", callback_data=f"dev_ack_export_xlsx:{event_id}")]
    ]

    # Інлайн-кнопки магазинів з пагінацією
    SHOPS_PER_PAGE = 6
    pages_total = max(1, (len(shops_summary) + SHOPS_PER_PAGE - 1) // SHOPS_PER_PAGE)
    page = max(0, min(page, pages_total - 1))

    start_idx = page * SHOPS_PER_PAGE
    paginated_shops = shops_summary[start_idx:start_idx + SHOPS_PER_PAGE]

    for idx_offset, s in enumerate(paginated_shops):
        actual_shop_idx = start_idx + idx_offset
        shop_name = s['shop']
        btn_text = f"🏪 {shop_name} (✅ {s['ack_count']}/{s['total_count']})"
        buttons.append([InlineKeyboardButton(
            text=btn_text,
            callback_data=f"dev_ack_shop:{event_id}:{actual_shop_idx}:{page}"
        )])

    if pages_total > 1:
        nav_row = []
        if page > 0:
            nav_row.append(InlineKeyboardButton(text="⬅️", callback_data=f"dev_ack_ev:{event_id}:{page - 1}"))
        nav_row.append(InlineKeyboardButton(text=f"📄 {page + 1}/{pages_total}", callback_data="ignore"))
        if page < pages_total - 1:
            nav_row.append(InlineKeyboardButton(text="➡️", callback_data=f"dev_ack_ev:{event_id}:{page + 1}"))
        buttons.append(nav_row)
...
async def developer_ack_shop_users_view(callback: CallbackQuery):
    """Детальний список співробітників конкретного магазину (керівник, працівники, стажери)."""
    if not await _ensure_developer(callback, allow_observer=True):
        return

    # Format: dev_ack_shop:{event_id}:{shop_idx}:{page}
    parts = callback.data.split(":")
    event_id = int(parts[1])
    ...
    event = await get_event_by_id(event_id)
    shops_summary = await get_event_shops_summary(event_id)
    if not event or shop_idx >= len(shops_summary):
        await callback.answer("Дані не знайдено.", show_alert=True)
        return

    shop_name = shops_summary[shop_idx]['shop']
    ...
    def _format_user_line(u: Dict[str, Any]) -> str:
        ...
        if u.get('acknowledged_at'):
            ...
            try:
                dt_ack = datetime.fromisoformat(ack_raw.replace("Z", "+00:00"))
...
async def developer_ack_export_xlsx_callback(callback: CallbackQuery):
    """Генерує та відправляє XLSX звіт по події ознайомлення."""
    if not await _ensure_developer(callback, allow_observer=True):
        return

    # Format: dev_ack_export_xlsx:{event_id}
    parts = callback.data.split(":")
    event_id = int(parts[1])
    ...
    try:
        excel_buf = await generate_material_ack_xlsx(event_id)
        role_clean = re.sub(r'[^\w\-]', '_', event['role'])
        date_str = datetime.now().strftime("%Y%m%d")
        filename = f"oznayomlennya_{role_clean}_day_{event['day']}_{date_str}.xlsx"

        doc = BufferedInputFile(excel_buf.getvalue(), filename=filename)
        await callback.message.answer_document(
            document=doc,
            caption=(
                f"📊 <b>Звіт про ознайомлення з матеріалами</b>\n"
                f"📚 Посада: <b>{event['role']}</b> | 📅 День: <b>{event['day']}</b>\n"
                f"🏪 Окремі аркуші по кожному магазину + Зведений звіт."
            )
        )
    except Exception as e:
        logger.exception(f"Помилка експорту XLSX для події {event_id}: {e}")
...
async def _rebuild_user_profile_view(callback: CallbackQuery, user_id: int, state: FSMContext, back_callback: str = "dev_users_menu", ret_code: str = "menu"):
    """Helper to refresh the user profile view after an action."""
    await _show_user_profile_card(callback, user_id, state=state, back_callback=back_callback, ret_code=ret_code)

# --- Learning Days Management ---

async def developer_days_manage_menu(callback: CallbackQuery, state: FSMContext, user_id: Optional[int] = None, ret_code: str = "menu"):
    if user_id is None:
        try:
            parts = callback.data.split(":")
            user_id = int(parts[1])
    ...
    for day, status in overview:
        ...
        buttons.append([InlineKeyboardButton(
            ...
            callback_data=f"dev_toggle_day:{user_id}:{day}:{action}:{ret_code}"
        )])
    buttons.append([InlineKeyboardButton(text="⬅️ Назад до профілю", callback_data=f"dev_user_profile_back:{user_id}:{ret_code}")])
...
async def developer_toggle_day_status(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        day = int(parts[2])
        action = parts[3]
        ret_code = parts[4] if len(parts) > 4 else "menu"
    except (ValueError, IndexError):
    ...
    # Refresh the menu
    await developer_days_manage_menu(callback, state, user_id=user_id, ret_code=ret_code)
...
async def developer_user_profile_back(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    if ret_code == "mgr_team" or ret_code.startswith("mgr_team_"):
        has_access, is_admin, is_territorial = await _check_access(callback)
        is_obs = await is_observer_user(callback.from_user.id)
        card_data = await _build_manager_card(user_id, is_admin, is_territorial, is_observer=is_obs, bot=callback.bot)
        if card_data:
            text, kb = card_data
            photo_input = await get_user_avatar_input(callback.bot, user_id)
            await _send_or_edit_card_photo(callback, photo_input, text, reply_markup=kb)
            return
    await _rebuild_user_profile_view(callback, user_id, state, ret_code=ret_code)
    try:
        await callback.answer()
    except Exception:
        pass
...
async def developer_user_modify_menu(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⬅️ Назад до профілю", callback_data=f"dev_user_profile_back:{user_id}:{ret_code}")]
    ])
    await _send_text_screen(callback, "✍️ <b>Що саме ви хочете змінити?</b>", reply_markup=kb)
    await callback.answer()
...
async def developer_user_delete_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="❌ Ні, назад", callback_data=f"dev_user_profile_back:{user_id}:{ret_code}")
        ]
    ])
    await _send_text_screen(callback, f"Ви впевнені, що хочете видалити користувача <code>{user_id}</code>?", reply_markup=kb)
    await callback.answer()
...
async def _render_training_promoted(callback: CallbackQuery, token: str, page: int = 0):
    ...
    if not is_obs:
        kb_rows.append([InlineKeyboardButton(text="📥 Вивантажити", callback_data=f"dev_training_promoted_export:{token}")])
    kb_rows.append([InlineKeyboardButton(text="⬅️ Навчальний процес", callback_data="dev_training_menu")])
    kb = InlineKeyboardMarkup(inline_keyboard=kb_rows)
    await _edit_or_answer(callback.message, "\n".join(lines), reply_markup=kb)
...
async def dev_pos_edit_days_start(callback: CallbackQuery, state: FSMContext):
    ...
    if not pos:
        await callback.answer("Посаду не знайдено!", show_alert=True)
        return
    if pos['name'] == 'Керівник':
        from database.hr import is_developer_user
        if not await is_developer_user(callback.from_user.id):
            await callback.answer("⛔️ Змінювати кількість днів для посади «Керівник» може лише адміністратор!", show_alert=True)
            return
...
async def dev_pos_edit_days_process(message: Message, state: FSMContext):
    ...
    pos_id = data.get('edit_pos_id')
    pos = await get_position_by_id(pos_id)
    if pos and pos['name'] == 'Керівник':
        from database.hr import is_developer_user
        if not await is_developer_user(message.from_user.id):
            await message.answer("⛔️ Змінювати кількість днів для посади «Керівник» може лише адміністратор!")
            await state.clear()
            return
    ...
    from database.hr import is_developer_user
    is_admin = await is_developer_user(message.from_user.id)
    buttons = [
        [InlineKeyboardButton(text="✏️ Змінити назву", callback_data=f"dev_pos_edit_name:{pos_id}")],
    ]
    if pos['name'] != 'Керівник' or is_admin:
        buttons.append([InlineKeyboardButton(text="✏️ Змінити кількість днів", callback_data=f"dev_pos_edit_days:{pos_id}")])
    buttons.append([InlineKeyboardButton(text="🔙 До списку посад", callback_data="dev_positions_menu")])
    await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def _build_territorials_team_view(bot: Optional[Bot] = None, is_observer: bool = False) -> tuple[str, InlineKeyboardMarkup]:
...
async def developer_territorials_menu(callback: CallbackQuery):
    if not await _ensure_developer(callback, allow_observer=True):
        return
    is_obs = await is_observer_user(callback.from_user.id)
    text, kb = await _build_territorials_team_view(bot=callback.bot, is_observer=is_obs)
    await _show_admin_photo_menu(
        callback,
        "admin_spus.jpg",
        text,
...
async def developer_territorial_view(callback: CallbackQuery):
    ...
    if not is_obs:
        buttons.append([InlineKeyboardButton(text="🗑 Видалити", callback_data=f"dev_territorial_del:{t_uid}")])
...
async def _build_observers_menu_view(page: int = 0, bot: Optional[Bot] = None, is_observer: bool = False) -> tuple[str, InlineKeyboardMarkup]:
...
async def developer_observers_menu(callback: CallbackQuery):
    ...
    is_obs = await is_observer_user(callback.from_user.id)
    text, kb = await _build_observers_menu_view(page=page, bot=callback.bot, is_observer=is_obs)
...
async def developer_observer_view(callback: CallbackQuery):
    ...
    buttons = []
    if not is_obs:
        buttons.append([InlineKeyboardButton(text="🗑 Видалити наглядача", callback_data=f"dev_obs_del_confirm:{obs_uid}")])
    buttons.append([InlineKeyboardButton(text="⬅️ До списку наглядачів", callback_data="dev_observers_menu")])
...
def register_developer_menu_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(developer_list_interns_no_manager, lambda c: c.data == "dev_users_interns_no_mgr")
    dp.callback_query.register(developer_list_workers_no_manager, lambda c: c.data == "dev_users_workers_no_mgr")
    ...
    dp.callback_query.register(developer_user_card_back_handler, lambda c: c.data and c.data.startswith("dev_u_cback:"))
    ...
    dp.callback_query.register(developer_ack_events_list_view, lambda c: c.data and c.data.startswith("dev_ack_events_list:"))
    dp.callback_query.register(developer_ack_event_card_view, lambda c: c.data and c.data.startswith("dev_ack_ev:"))
    dp.callback_query.register(developer_ack_shop_users_view, lambda c: c.data and c.data.startswith("dev_ack_shop:"))
    dp.callback_query.register(developer_ack_changes_view, lambda c: c.data and c.data.startswith("dev_ack_changes:"))
    dp.callback_query.register(developer_ack_export_xlsx_callback, lambda c: c.data and c.data.startswith("dev_ack_export_xlsx:"))
