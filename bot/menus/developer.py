...
async def developer_ack_event_card_view(callback: CallbackQuery):
    ...
    text = (
        ...
        f"📝 Зміна: {desc_preview}\n"
        f"🕒 Дата розсилки: {dt_formatted}\n\n"
        f"📊 <b>Загальний прогрес:</b>\n"
        f"• Ознайомились: {overall_stats['acknowledged_count']} з {overall_stats['total']} ({overall_stats['ack_percent']}%)\n"
        f"• Не ознайомились: {overall_stats['unacknowledged_count']} ({overall_stats['unack_percent']}%)\n"
        f"• Із запізненням (>72 год): {overall_stats['acknowledged_late_count']}\n"
        f"───────────────────\n"
